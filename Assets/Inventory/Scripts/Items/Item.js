...
function PickUpItem ()
{
	...
	playersinv.gameObject.SendMessage ("PlayPickUpSound", SendMessageOptions.DontRequireReceiver); //Play sound
		...
		if(stackable){
		    var locatedit:Item;
		    var thisItemName = this.transform.GetComponent(Item).itemType;
			for(var t:Transform in playersinv.Contents){
                var otherItemName = t.GetComponent(Item).itemType;
				if(otherItemName==thisItemName){//if the item we wanna stack this on has the same name
